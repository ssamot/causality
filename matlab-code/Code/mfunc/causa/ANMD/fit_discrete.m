function [fct, p_val, stat]=fit_discrete(X,Y,level,doplots,dir)

fp=0; % Change to 2 to see the warnings
stat = nan;
...
%    display('okokokokokoko')
    ...
    if length(unique(eps))==1
        dprintf(fp, 'Warning!! there is a deterministic relation between X and Y');
        p_val=1;
    else
        [p_val,stat]=chi_sq_quant(eps,X,length(unique(eps)),length(X_values));
    end
    if doplots==1
        %fct
        %p_val
